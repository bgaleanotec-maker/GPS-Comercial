...
def manage_users():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Usuario "{form.full_name.data}" creado como {role}.', 'success')
    ...
    return render_template('user_management/manage_users.html',
                           title='Gestionar Usuarios', form=form,
...
def edit_user(user_id):
    """Editar usuario completo: datos, rol, direcciones, estado, telefono."""
    if current_user.role != 'admin':
        abort(403)
